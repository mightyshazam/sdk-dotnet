...
namespace Temporal.Util
{
    internal class RuntimeEnvironmentInfo
    {
        ...
        private static string GetRuntimeName(CoreAssemblyInformation coreAssembyInfo)
        {
            ...
            else if (frameworkDescription.StartsWith(".NET 6", StringComparison.OrdinalIgnoreCase))
            {
                runtimeName = ".NET 6";
            }
#endif
        ...
        public override string ToString()
        {
            ...
            if (stringView == null)
            {
                stringView = $"{RuntimeName} version {RuntimeVersion} ({ProcessArchitecture}) running on {OsPlatform} {OsArchitecture}";
